...
test_xyzrph = {'antenna_x': {'1503413148': '-0.889', '1503423148': '-0.889', '1503443148': '-0.889'},
               'antenna_y': {'1503413148': '-0.923', '1503423148': '-0.923', '1503443148': '-0.923'},
               'antenna_z': {'1503413148': '-4.193', '1503423148': '-4.193', '1503443148': '-4.193'},
...
test_xyzrph_dual = {'antenna_x': {'1583305645': '2.569', '1583315645': '2.569', '1583335645': '2.569'},
                    'antenna_y': {'1583305645': '-2.978', '1583315645': '-2.978', '1583335645': '-2.978'},
                    'antenna_z': {'1583305645': '-12.975', '1583315645': '-12.975', '1583335645': '-12.975'},
...
class OptionsWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        configtxt = QtWidgets.QLabel('Config: ')
        ...
        for lbl in [configtxt, self.vess_descrip, time_descrip]:
            config_layout_labels.addWidget(lbl)
        for widg in [self.config_name, self.vess_select, self.time_select]:
...
class VesselWidget(QtWidgets.QWidget):
    ...
    def setup_toolbar(self):
        ...
        newconfig.triggered.connect(self.import_from_kongsberg)
        ...
        importsingle = QtWidgets.QAction('Load dataset', self)
        importdual = QtWidgets.QAction('Load dualhead dataset', self)
    ...
    def update_xyzrph_vessel(self, pth_to_vessel):
        ...
        if self.xyzrph is not None:
            first_sensor = list(self.xyzrph.keys())[0]
            tstmps = list(self.xyzrph[first_sensor].keys())
            vess_entry = {tstmp: pth_to_vessel for tstmp in tstmps}
            self.xyzrph['vessel_file'] = vess_entry
                    ...
                    if entry is not None:
                        try:
                            if entry in self.xyzrph:
                                if float(self.xyzrph[entry][orig_tstmp]) != float(data[cnt]):
                                    self.xyzrph[entry][orig_tstmp] = format(data[cnt], '.3f')
                            else:
    ...
    def populate_from_xyzrph(self, xyzrph):
        ...
        """
        self.xyzrph = xyzrph
        self.opts_window.populate_from_xyzrph(xyzrph)
        ...
        if fil:
            ...
            if posxyzrph is not None:
                if self.xyzrph is not None:
                    ...
                    for sens in sensors:
                        for tstmp in self.xyzrph[sens]:
                            if float(posxyzrph[sens]):
                                self.xyzrph[sens][tstmp] = str(posxyzrph[sens])
                    self.opts_window.populate_from_xyzrph(self.xyzrph)
                else:
                    print('Expect data to exist before loading from POS MV, please make a new config or open config')
            else:
    ...
    def import_from_kongsberg(self):
        ...
        if fil:
            ...
            mbesxyzrph = return_xyzrph_from_mbes(fil)
            if mbesxyzrph is not None:
                self.populate_from_xyzrph(mbesxyzrph)
            else:
                print('Unable to load from {}'.format(fil))
        else:
            print('Import cancelled')
    ...
    def save_configuration(self):
        ...
        if fil:
            if self.xyzrph is not None:
                first_sensor = list(self.xyzrph.keys())[0]
                tstmps = list(self.xyzrph[first_sensor].keys())
                if 'vessel_file' not in self.xyzrph:
                    vess_entry = {tstmp: self.vessview_window.pth_to_vessel_file for tstmp in tstmps}
                    self.xyzrph['vessel_file'] = vess_entry
                if 'vess_center_x' not in self.xyzrph:
                    vess_entry = {tstmp: '0.000' for tstmp in tstmps}
                    self.xyzrph['vess_center_x'] = vess_entry
                if 'vess_center_y' not in self.xyzrph:
                    vess_entry = {tstmp: '0.000' for tstmp in tstmps}
                    self.xyzrph['vess_center_y'] = vess_entry
                if 'vess_center_z' not in self.xyzrph:
                    vess_entry = {tstmp: '0.000' for tstmp in tstmps}
                    self.xyzrph['vess_center_z'] = vess_entry
                if 'vess_center_r' not in self.xyzrph:
                    vess_entry = {tstmp: '0.000' for tstmp in tstmps}
                    self.xyzrph['vess_center_r'] = vess_entry
                if 'vess_center_p' not in self.xyzrph:
                    vess_entry = {tstmp: '0.000' for tstmp in tstmps}
                    self.xyzrph['vess_center_p'] = vess_entry
                if 'vess_center_yaw' not in self.xyzrph:
                    vess_entry = {tstmp: '0.000' for tstmp in tstmps}
                    self.xyzrph['vess_center_yaw'] = vess_entry
    ...
    def load_from_existing_xyzrph(self):
        ...
        try:
            vess = self.xyzrph['vessel_file'][first_tstmp]
            ...
            self.opts_window.vessel_selected()
            pos = [float(self.xyzrph['vess_center_x'][first_tstmp]), float(self.xyzrph['vess_center_y'][first_tstmp]),
                   float(self.xyzrph['vess_center_z'][first_tstmp]), float(self.xyzrph['vess_center_r'][first_tstmp]),
                   float(self.xyzrph['vess_center_p'][first_tstmp]), float(self.xyzrph['vess_center_yaw'][first_tstmp])]
            ...
            self.opts_window.vessel_selected()
        self.populate_from_xyzrph(self.xyzrph)
