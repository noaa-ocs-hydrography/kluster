...
class Filter(BaseFilter):
    ...
    def _run_algorithm(self, percent_depth: float):
        print(f'Running second_return ({percent_depth}) on {self.fqpr.output_folder}')
        if 'z' not in self.fqpr.multibeam.raw_ping[0]:
            raise ValueError(f'second_return: unable to find georeferenced depths, have you processed this data?')
        ...
        for cnt, rp in enumerate(self.fqpr.multibeam.raw_ping):
            ...
            filt_nadir[0:8] = filt_nadir[9]  # extend the rolling min to cover the missing windowed values
