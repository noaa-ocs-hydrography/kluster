...
def evaluate_raw_backscatter(fle):
    ...
    detection_info = data['ping'][
        'detectioninfo']  # detction info : re - accepted, 2 = rejected, 1 = phase detection, 0 = amplitude detection.
...
def create_raw_bs_evaluation(fle_dir):
    ...
    for q in range(len(fles)):
        ...
        # bad_beam_arr[bad_beam_arr == 2] = np.nan
        ax[1].imshow(bad_beam_arr, aspect='auto', interpolation='none', vmin=0, vmax=1, cmap=cmap_rainbow,
                     label='Bad Soundings')
