...
def create_raw_bs_evaluation(fle_dir):
    ...
    for q in range(len(fles)):
        ...
        # bad_beam_arr[bad_beam_arr == 2] = np.nan
        ax[1].imshow(bad_beam_arr, aspect='auto', interpolation='none', vmin=0, vmax=1, cmap=cmap, label='Bad Soundings')
...
def prepare_backscatter_for_file(fle):
    ...
    # reflectivity1 = data['ping']['reflectivity1'] #added this datagram
    ...
    reflectivity_improved = reflectivity - tvg_rt + tvg_improved
    ...
    reflectivity1_binned = np.empty((num_pings, len(beam_angle_bins)))
    reflectivity1_binned[:,:] = np.nan
    sectors_binned = np.empty((num_pings, len(beam_angle_bins)))
    sectors_binned[:,:] = np.nan
    backscatter_binned = np.empty((num_pings, len(beam_angle_bins)))
    backscatter_binned[:,:] = np.nan
    ...
    for n in range(num_pings):
        ...
        reflectivity_ping = reflectivity[n] #THIS IS prob WRONG removing real-time tvg, adding spreading loss, absopriton loss (assuing units are db/km)
        ...
        backscatter_binned[n, bin_ind1:bin_ind2] = interp1d(beam_angle_ping, backscatter[n])(beam_angle_bins[bin_ind1:bin_ind2])
    ...
    backscatter_mean = np.mean(backscatter_binned, axis=0)
    ...
    num_pings = np.shape(backscatter_binned)[0]
    num_bins = np.shape(backscatter_binned)[1]
    backscatter_sectorized = np.empty((num_sectors, num_pings, num_bins))
    backscatter_sectorized[:, :, :] = np.nan

    for sector in range(num_sectors):
            for ping in range(num_pings):
                for bin in range(num_bins):
                    if sectors_binned[ping, bin] == sector:
                        backscatter_sectorized[sector, ping, bin] = backscatter_binned[ping, bin]
                    else:
                        backscatter_sectorized[sector, ping, bin] = np.nan

    results = [{'mode': mode}, {'backscatter_binned': backscatter_binned},
                         {'backscatter_sectorized': backscatter_sectorized},
                         {'sectors_binned': sectors_binned}, {'r_binned_mean': r_binned_mean},
                         {'beam_angle_bins': beam_angle_bins}]

    return data, results


def package_backscatter_from_fles(fles):
    data_dict = {}
    results_dict = {}

    for fle in fles:
        data, results = prepare_backscatter_for_file(fles)
        data_dict[fle] = data
...
def backscatter_correction(results_dict, lc_nnic, bs_scalar_mean):
    fles = list(results_dict.keys())
    modes = []
    beam_angle_bins = results_dict[fles[0]][5]['beam_angle_bins']
    ...
    for mode in modes:
        fle_mode = []
        for fle in fles:
            if results_dict[fle][0]['mode'] == mode:
                fle_mode.append(fle)
    ...
    for mode in modes:
        ...
        if len(fles) != 2:
            print('Did not find 2 files for ' + mode + ' mode. Cannot complete calibration. Check directory.')

        bs_1 = results_dict[fles[0]][2]['backscatter_sectorized']
        bs_2 = results_dict[fles[1]][2]['backscatter_sectorized']
        bs_1_mean = np.nanmean(bs_1, axis=1)
        bs_2_mean = np.nanmean(bs_2, axis=1)
        bs_mean = (bs_1_mean+bs_2_mean)/2
        bs_mean_corr = bs_mean - lc_nnic #corrected mean backscatter for a given mode as a function of transducer relative angle
...
def plot_curves(mode_dict):
    ...
    for n in range(len(modes)):
        ...
        for m in range(len(np.shape(bs_corr)[0])):
...
if __name__ == "__main__":
    ...
    fle_dir = r'D:\Backscatter\OPR-T381-RA-22_Mariana_Islands\H13578\MBES\S221_Simrad-EM710_ICE\2022-140'
